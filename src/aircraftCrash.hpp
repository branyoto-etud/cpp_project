...
public:
    AircraftCrash(const std::string& flight_number, const Point<3, float>& pos,
                  const Point<3, float>& speed, const AircraftCrashReason& reason)
        : std::runtime_error {build_error_msg(flight_number, pos, speed, reason)}
    {
        assert(!flight_number.empty());
    }
private:
    static std::string build_error_msg(const std::string& flight_number, const Point<3, float>& pos,
                                       const Point<3, float>& speed, const AircraftCrashReason& reason) {
        std::string msg;
